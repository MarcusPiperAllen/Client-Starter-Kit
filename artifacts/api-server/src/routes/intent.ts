...
- "callToActionCustom": for specific conversion phrases with no standard match, capture the phrase here (e.g. "Join Waitlist" when the user says "join the waitlist" or "waitlist signup", "Get Early Access" for early access, or any custom phrase explicitly stated). Use "" only when no specific custom phrase was given. VOLUNTEER RULE: when the project's primary conversion action is volunteering and no donation/giving/support language is present, ALWAYS set callToAction to "get involved" and callToActionCustom to "Volunteer Today" — this is mandatory, not optional. PRIORITY RULE: when both donation/giving/support language AND volunteer language appear together, donation is the primary CTA — set callToAction to "donate" and do NOT set callToActionCustom to "Volunteer Today" unless the project is clearly volunteer-primary with no donation intent.
